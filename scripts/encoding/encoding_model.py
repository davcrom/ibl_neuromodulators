...
def split_pose(pose: nap.TsdFrame) -> dict[str, nap.TsdFrame]:
    ...
    The pose frame holds paired coordinate columns named ``{keypoint}_x...`` and
    ``{keypoint}_y...`` (e.g. ``pupil_top_r_x_ens_median``). Columns are grouped
    by keypoint and each pair is emitted as its own 2-column block.

    Args:
        pose (nap.TsdFrame): pose traces; columns carry an ``_x``/``_y``
            coordinate token, optionally followed by a suffix.

    Returns:
        dict[str, nap.TsdFrame]: keypoint name -> (len(pose), 2) block on the
        pose time base, with columns ``["x", "y"]``. Keypoints missing either
        coordinate are skipped.
    """
    # group coordinate columns by keypoint (the column up to its _x/_y token),
    # preserving first-seen order
    coordinates: dict[str, dict[str, str]] = {}
    for column in pose.columns:
        match = re.search(r"_([xy])(?=_|$)", column)
        if match is None:
            continue
        keypoint = column[: match.start()]
        coordinates.setdefault(keypoint, {})[match.group(1)] = column
