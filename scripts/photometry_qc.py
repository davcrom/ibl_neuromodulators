...
Usage:
    python scripts/photometry_qc.py
"""
import sys
...
# Build group from session catalog
...
# Explode to one row per recording with target_NM, brain_region, hemisphere
df_rec = group.recordings
...
df_qc = df_qc.merge(
    df_rec[['eid', 'brain_region', 'target_NM', 'subject', 'session_type', 'start_time']],
    on=['eid', 'brain_region'],
    how='inner',
)
df_qc['date'] = pd.to_datetime(df_qc['start_time'], format='ISO8601').dt.date
print(f"After merge with recordings: {len(df_qc)} signals from {df_qc['eid'].nunique()} sessions")
...
targets = [t for t in VALID_TARGETNMS if t in df_qc['target_NM'].unique()]
