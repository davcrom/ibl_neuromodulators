...
Sessions are labelled with `config.SESSION_GROUPS`, which sets every figure's
colors and stacking order.

Usage:
    python scripts/dataset_overview.py [--session_x {session_n,day_n}]
                                       [--horizontal]
"""
import argparse
import sys
...
from iblnm.config import (
    ...
    TARGETNMS_TO_ANALYZE, POSE_FPATH, SESSION_GROUPS,
)
from iblnm.data import PhotometrySessionGroup
from iblnm.util import concat_logs, deduplicate_log, label_session_groups
...
# Label once, here: every group rebuilt below carries the column, and the
# fraction_correct and contrasts columns multi-column criteria may name are
# already on `df` from the scan above.
df = label_session_groups(df, SESSION_GROUPS)
...
ax = session_overview_matrix(grp, columns=xcol)
...
if n_rec > 0:
    ax = target_overview_barplot(grp.recordings, horizontal=args.horizontal)
