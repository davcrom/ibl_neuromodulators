...
class TestPlotOlsDropone:
    ...
    def _df(self, targets=('VTA-DA', 'DR-5HT'),
            events=('feedback_times', 'stimOn_times')):
        """Long-form per-session ΔR²: 2 mice per target-NM, 3 sessions each.
    ...
    @staticmethod
    def _points(ax):
        """All scatter-point (x, y) offsets drawn on an axis."""
        from matplotlib.collections import PathCollection
        return np.concatenate([c.get_offsets() for c in ax.collections
