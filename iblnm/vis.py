...
def _dropone_rows(label, predictor=None):
    """Grid row + shared y-label for a drop-one ΔR² figure.

    Parameters
    ----------
    label : str
        The row label: the dropped term a per-term figure covers, or the
        target-NM a per-target figure covers.
    predictor : str or None
        The ``config.RESPONSE_DROPPED_TERMS`` label the row reads. ``None`` for
        a grid grouped by predictor, where each x group reads its own label's
        rows instead.
    ...
    supylabel : str
    """
    return ([(label, 'delta_r2_adj', predictor)],
...
def _pool_by_group(df_cell, value_col, groups, group_col='target_NM'):
    """Pool a panel's per-session values by the values of one column.

    Collects ``value_col`` across every session (all subjects) of each value in
    ``groups``, dropping groups with no rows in this panel.

    Parameters
    ----------
    df_cell : pd.DataFrame
        One panel's rows; needs ``group_col`` and ``value_col``. Each row is one
        session.
    value_col : str
        Column of per-session values to pool (e.g. ``delta_r2`` or ``r2``).
    groups : sequence of str
        ``group_col`` values in plot order.
    group_col : str
        Column the sessions are pooled by — target-NM by default, drop-one
        label for a figure fixed to one target.

    Returns
    -------
    dict[str, np.ndarray]
        ``group value -> pooled per-session values``, in ``groups`` order,
        omitting groups with no values in this panel.
    """
    pooled = {group: df_cell.loc[df_cell[group_col] == group, value_col].values
              for group in groups}
    return {group: vals for group, vals in pooled.items() if len(vals)}
...
def plot_ols_dropone_target(df, title, target_nm, terms, ylim=None,
                            mouse_pvalues=None,
                            alpha=PERSESSION_SIGNIFICANCE_ALPHA):
    """Per-session drop-one ΔR² for one target-NM — drop-one labels along x.

    The per-term figure rearranged: ``plot_ols_dropone`` fixes the dropped term
    and groups subjects by target-NM, this fixes the target-NM and groups its
    subjects by dropped term, so one target's terms read against each other.
    Marks and significance coloring are ``plot_ols_dropone``'s. See
    ``_persession_subject_grid``.

    Parameters
    ----------
    df : pd.DataFrame
        The per-recording OLS frame (``config.OLS_PERSESSION_COLUMNS``); rows
        of other target-NMs are ignored.
    title : str
        Figure suptitle.
    target_nm : str
        The target-NM this figure covers.
    terms : sequence of str
        Drop-one labels, one x group each, in plot order — typically
        ``DROPONE_TERM_CLASSES['main']``.
    ylim : tuple[float, float] or None
        Shared y-axis range. ``None`` autoscales to this target's data.
    mouse_pvalues : pd.DataFrame or None
        Per-mouse permutation results, graying a non-significant subject's mean
        dash (see ``plot_ols_dropone``).
    alpha : float
        False-discovery-rate threshold; a mark keeps its color when
        ``q_value < alpha``.
    """
...
def plot_ols_dropone_target_subject(df, title, target_nm, terms, ylim=None):
    """Per-session drop-one ΔR² for one target-NM — median + Q1–Q3 per subject.

    Same figure as ``plot_ols_dropone_target`` (drop-one labels along x, one
    target's subjects in each) but each subject is drawn as its median with an
    interquartile whisker, as in ``plot_ols_dropone_subject``. See
    ``_persession_subject_grid``.
    """
    rows, supylabel = _dropone_rows(target_nm)
    return _persession_subject_grid(df, title, rows, supylabel,
...
def _persession_target_grid(df, title, rows, supylabel, ylim=None,
    ...
    violin per x-slot. Each entry of ``rows`` is one grid row; columns are
    events (``_sort_events`` order). Within a panel each ``group_col`` value —
    target-NM by default, drop-one label for a figure fixed to one target —
    occupies one x-slot, drawn as a violin of the pooled per-session values
    across all its subjects (see ``_pool_by_group``). A violin is faced in the
    ``TARGETNM_COLORS`` entry of its own rows' target-NM, so a figure grouped by
    label draws every violin in the selected target's color. One x-tick per
    group. All panels share one y-axis. An empty frame returns a titled figure.
    ...
    rows : list[tuple[str, str, str | None]]
        ``(row_label, value_column, predictor)`` per grid row. ``predictor``
        selects the frame rows to read, unless ``group_col`` is
        ``'predictor'``, in which case each group's own value selects them.
    supylabel : str
        Shared y-axis label.
    ylim : tuple[float, float] or None
        ``(bottom, top)`` for the shared y-axis. ``None`` autoscales to the
        panels' own data.
    group_col : str
        Column the x-slots are taken from.
    group_order : sequence of str or None
        ``group_col`` values in plot order, one slot and tick each. ``None``
        sorts the values present by ``TARGETNM2POSITION``, the target-NM order.
    select : dict[str, str] or None
        Column -> value every plotted row must equal, applied before layout —
    ...
    fig, axes = plt.subplots(
        ...
        layout='constrained',
        figsize=(1.2 * len(group_order) * n_cols + 1, 2.0 * n_rows + 1))

    for c, event in enumerate(events):
        df_event = df[df['event'] == event]
        for r, (label, value_col, predictor) in enumerate(rows):
            ax = axes[r, c]
            # Grouped by predictor, each violin reads its own label's rows
            # rather than the row's predictor.
            ...
            if ylim is not None:
                ax.set_ylim(ylim)
        axes[-1, c].set_xticks(range(len(group_order)))
        axes[-1, c].set_xticklabels(group_order, rotation=30, ha='right',
                                    fontsize=TICKFONTSIZE)
    fig.supylabel(supylabel)
    fig.suptitle(title, fontsize=LABELFONTSIZE)
    return fig
...
def plot_ols_dropone_target_violin(df, title, target_nm, terms, ylim=None):
    """Per-session drop-one ΔR² for one target-NM — one violin per label.

    Same figure as ``plot_ols_dropone_target`` (drop-one labels along x, one
    target's sessions in each) but each label is drawn as a violin of that
    target's per-session ΔR² pooled across its subjects, as in
    ``plot_ols_dropone_violin``. Every violin takes the target's color.
    ``ylim`` fixes the shared y-axis. See ``_persession_target_grid``.
    """
    rows, supylabel = _dropone_rows(target_nm)
    return _persession_target_grid(df, title, rows, supylabel, ylim=ylim,
                                   group_col='predictor', group_order=terms,
...
def _summarize_state_posteriors(
    ...
    states : pandas.DataFrame
        One mouse's per-trial DDM-HMM state frame: integer assigned ``state``
        and float posterior columns ``p_state_1``…``p_state_K`` (each in [0, 1]).
    bins : numpy.ndarray
    ...
    occupancy : pandas.Series
        Indexed by integer state label (1…K); fraction of trials assigned to
        each state. Sums to 1.
    histograms : dict of int to numpy.ndarray
        Maps each state label to the counts of that state's posterior column
        across ``bins`` (NaNs excluded).
    """
    labels = sorted(int(c.removeprefix('p_state_')) for c in states.columns
                    if c.startswith('p_state_'))
    assigned = states['state'].dropna().astype(int)
    occupancy = assigned.value_counts().reindex(labels, fill_value=0) / len(assigned)
    histograms = {label: np.histogram(states[f'p_state_{label}'].dropna(),
                                      bins=bins)[0]
                  for label in labels}
    return occupancy, histograms


# Shared layout for the per-mouse state figures.
ROW_HEIGHT = 2.4   # inches per panel row, so axis heights match across figures
POINT_ALPHA = 0.5  # alpha for empirical data-point markers


def draw_state_posteriors(ax, states: pd.DataFrame, bins: np.ndarray) -> None:
    """Step histogram of every state's posterior, with an occupancy inset.

    Crisp assignments concentrate mass near 0 and 1. The inset bars each
    state's share of assigned trials (:func:`_summarize_state_posteriors`).
    States are colored by position (``plt.cm.tab10``).
    ...
    ax : matplotlib.axes.Axes
        Axes to draw on.
    states : pandas.DataFrame
        One mouse's ``state`` plus ``p_state_1``…``p_state_K`` columns.
    bins : numpy.ndarray
        Histogram bin edges spanning [0, 1].
    """
    occupancy, histograms = _summarize_state_posteriors(states, bins)
...
def draw_state_dwell(ax, dwell: pd.DataFrame, n_bins: int = 20) -> None:
    """Step histogram of run lengths (dwell times, in trials) per state.

    Parameters
    ----------
    ax : matplotlib.axes.Axes
        Axes to draw on.
    dwell : pandas.DataFrame
        One row per run, columns ``['state', 'length']``, from
        :func:`iblnm.analysis.state_dwell_times`.
    n_bins : int, optional
        Histogram bins spanning 0 to the longest run (default 20).
    """
    dwell_bins = np.linspace(0, dwell['length'].max(), n_bins + 1)
    by_state = dwell.groupby('state')['length']
    for (label, lengths), color in zip(by_state,
                                       plt.cm.tab10(np.arange(by_state.ngroups))):
        counts, _ = np.histogram(lengths, bins=dwell_bins)
        ax.stairs(counts, dwell_bins, color=color, label=f'state {label}')
    ax.set_xlabel('dwell time (trials)', fontsize=8)
    ax.set_ylabel('runs', fontsize=8)
    ax.tick_params(labelsize=7)


def draw_state_psychometric(ax, psych: pd.DataFrame) -> None:
    """Per-state empirical P(choose right) with the fitted psychometric overlay.

    The overlay (``psychofit.erf_psycho_2gammas``, over ±100 % contrast) is
    drawn only where a state's parameters are finite: low-trial states return
    NaN fits.
    ...
    for state, points in psych.groupby('state'):
        ...
        if np.all(np.isfinite(params)):
            ax.plot(grid, psy.erf_psycho_2gammas(params, grid),
                    color=color, linewidth=1.5)
    ax.axhline(0.5, color='gray', linestyle='--', alpha=0.5)
    ax.set_xlabel('signed contrast (%)', fontsize=8)
...
def draw_state_chronometric(ax, chrono: pd.DataFrame) -> None:
    """Per-state median RT against signed contrast, by outcome and side.

    Plain lines through the empirical medians: correct trials filled and
    solid, incorrect open and dashed. Each stimulus side is its own line, so
    the two sides are not connected across zero contrast (two points at 0).

    Parameters
    ----------
    ax : matplotlib.axes.Axes
        Axes to draw on.
    chrono : pandas.DataFrame
        Columns ``['state', 'outcome', 'side', 'signed_contrast',
        'median_rt']``; ``outcome`` in ``{'correct', 'incorrect'}``, ``side``
        in ``{'left', 'right'}``.
    """
    states = sorted(chrono['state'].unique())
    state_colors = dict(zip(states, plt.cm.tab10(np.arange(len(states)))))
    for (state, outcome, _), points in chrono.groupby(['state', 'outcome', 'side']):
        color = state_colors[state]
        style = _CHRONO_OUTCOME_STYLE[outcome]
        points = points.sort_values('signed_contrast')
        ax.plot(points['signed_contrast'], points['median_rt'],
    ...
    ax.set_xlabel('signed contrast (%)', fontsize=8)
    ax.set_ylabel('median RT (s)', fontsize=8)
    ax.tick_params(labelsize=7)
    ax.legend(handles=[
...
def plot_state_behavior(
    ...
    block_traces: Mapping[str, Mapping[str, np.ndarray] | None],
) -> plt.Figure:
    """One mouse's DDM-HMM behavioral figure, 3x2.

    Rows: (posterior histograms, dwell times), (psychometric, chronometric),
    then one block-transition trace panel per ``block_traces`` entry. States
    are colored by position within the mouse; each mouse is fit separately, so
    state labels carry no meaning across mice.
    ...
    subject : str
        Figure title.
    states : pandas.DataFrame
        ``state`` plus ``p_state_1``…``p_state_K``, one row per trial.
    dwell : pandas.DataFrame
        ``['state', 'length']``, one row per run.
    curves : mapping of str to pandas.DataFrame
        ``{'psychometric', 'chronometric'}`` frames, as the drawers read them.
    block_traces : mapping of str to mapping or None
        Two transition labels, in panel order, each to ``{'mean', 'sem'}`` of
        shape ``(2*window+1, K)`` or ``None`` when the mouse never made that
        transition, which leaves its panel blank.

    Returns
    -------
    matplotlib.figure.Figure
        Six axes, row-major, each titled by its panel.
    """
    fig, axes = plt.subplots(3, 2, figsize=(7, 3 * ROW_HEIGHT),
                             layout='constrained')
    ax_post, ax_dwell, ax_psych, ax_chrono, *ax_traces = axes.flat
    draw_state_posteriors(ax_post, states, np.linspace(0, 1, 21))
    draw_state_dwell(ax_dwell, dwell)
    draw_state_psychometric(ax_psych, curves['psychometric'])
    draw_state_chronometric(ax_chrono, curves['chronometric'])
    for ax, title in zip(axes.flat, ('state posteriors', 'dwell times',
                                     'psychometric', 'chronometric')):
        ax.set_title(title, fontsize=9)
    for ax, (label, stats) in zip(ax_traces, block_traces.items()):
